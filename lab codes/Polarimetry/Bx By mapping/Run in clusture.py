...
for i in range(len(Bz_arr)):
    ...
    Bz = Bz_arr[i]*1e6    # Bz = 1,2,3,4,...,100 *e6
    ...
    ellip_file_name = f"Bx_By_to_ellipticity_t_{t}_Bz_{Bz_arr[i]}.txt"
    np.savetxt(ellip_file_name, final_ellip_array, fmt='%f', delimiter='\t')
    
    
    farad_file_name = f"Bx_By_to_tan_faraday_rot_t_{t}_Bz_{Bz_arr[i]}.txt"
    np.savetxt(farad_file_name, final_faraday_rot, fmt='%f', delimiter='\t')
    
    
    chi_file_name = f"Bx_By_to_chi_t_{t}_Bz_{Bz_arr[i]}.txt"
    np.savetxt(chi_file_name, final_chi, fmt='%f', delimiter='\t')
    
    
    psi_file_name = f"Bx_By_to_psi_t_{t}_Bz_{Bz_arr[i]}.txt"
