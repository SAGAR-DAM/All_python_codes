...
t=np.linspace(0,1.7,len(e_files))
ellip_over_t=np.tan(np.linspace(0,np.pi/4,len(e_files)))
...
for i in range(len(e_files)):
    r = np.loadtxt(e_files[i], dtype=float, delimiter='\t')
    final_ellip_array=r[:][:]
...
plt.plot(t,calculated_Bx,'g--',label="Bx")
plt.xlabel("time (ps)")
plt.ylabel("Bx in MG")
...
plt.plot(t,calculated_By,'g--',label="By")
plt.xlabel("time (ps)")
plt.ylabel("By in MG")
