...
def density(x,y):
    ...
    if((25-3)*l0<=x<=(25+3)*l0):
        if(y<=12*l0):
            ...
            return(0.999*np.exp(-0.15*(y-12*l0)))
    if((75-6)*l0<=x<=(75+6)*l0):
        ...
        if(y<=12*l0):
            ...
            return(0.999*np.exp(-0.15*(y-12*l0))*(np.exp(-0.08*(abs(x-25*l0)-3*l0))+np.exp(-0.08*(abs(x-75*l0)-6*l0))))
...
plt.xlabel(r"X   (in $l_0$)")
plt.ylabel(r"Y   (in $l_0$)")
plt.colorbar()
plt.show()
