...
def find_index(array, value):
    ...
    absolute_diff = np.abs(np.array(array) - value)
...
imagepath = "D:\\data Lab\\ELI-NP March 2025\\27_03_2025\\separated out image\\27032025_TP_20_06_scan3_v500-[Phosphor]_flat.tif"
...
def extract_linecut(image, a, b, c, x_range=None, band=0, num_points=1000,logimage=False,log_image_cutoff=7, plot_logscale=False):
    ...
    axes[0].set_title(f"{imagepath[-48:-4]}"+"\nParabolic Linecut on Image")
    ...
    # axes[1].axvline(x = point_avg(x_values,10)[find_index(point_avg(intensities,10)[find_index(point_avg(x_values,10),250):find_index(point_avg(x_values,10), 500)],noise+max(intensities)/1e2)],color='r', linestyle='--', lw=2)
    tp_cut_pixel = 366
    tp_cutoff_energy = 29.53
