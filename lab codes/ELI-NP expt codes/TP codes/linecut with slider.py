...
# Load the image
image_path = "D:\\data Lab\\ELI-NP March 2025\\UPM50018_Kumar_csl\\CsI array\\01042025\\Image__2025-04-01__14-01-38.tiff"
...
# Function to plot the linecut with noise
def plot_linecut_with_noise(layer):
    global image, linecut_orientation

    # Define start and end points based on orientation
    ...
    # Compute linecut for current layer
    ...
    # Plot image with line and band
    ax1.imshow(np.log(image2[:, :, 0] + image2[:, :, 1] + image2[:, :, 2]), cmap="inferno")
    ax1.set_title("Image with Line and Band\n"+f"Layer:{(layer-25)//54}")
    ax1.axis("off")

    # Compute linecut data
    ...
    # Plot linecut with noise
    ax2.plot(x, y, color="blue")
    ax2.fill_between(x, y - y_noise, y + y_noise, color="red", alpha=0.3)
    ax2.set_title("Linecut with Band Averaging")
    ax2.set_xlabel("Position along the line")
    ax2.grid(which="both", color="k", lw=0.5)
...
# Initialize plot
initial_layer = 25
plot_linecut_with_noise(initial_layer)

# Add slider axis and create slider
ax_slider = plt.axes([0.2, 0.05, 0.6, 0.03])  # [left, bottom, width, height]
layer_slider = Slider(ax_slider, "Layer", 25, image.shape[0] - 1, valinit=initial_layer, valstep=54)

# Update function for slider
layer_slider.on_changed(plot_linecut_with_noise)
