...
A = np.array(Image.open("D:\\data Lab\\ELI-NP March 2025\\20-03-2025\\ESM data\\separated out image\\IP_Espec_20_03_2025_14_49-[Phosphor]_Surface_horizontal_part.tif"))
# A = A[500:,:]
...
Q = ndimage.gaussian_filter1d(np.sum(A, axis=0), 5)
Q = ndimage.gaussian_filter1d(Q, 5)
...
# BG1X = A[:, j-int(col1*0.5):j-int(col1*0.15)+int(col1*0.025)]
...
BG1 = np.zeros(shape=BG1.shape)
BG2 = np.zeros(shape=BG2.shape)
...
Energy_avg = moving_avg(Energy, 30)
NetSig_avg = moving_avg(NetSig, 30)

plt.figure()
plt.plot(Energy,NetSig,color='red')  
plt.plot(Energy_avg, NetSig_avg, 'k-')
# plt.plot(Energy,NoiseSig,color='blue')
plt.yscale('log')
# plt.xlim(0.1,np.max(Energy))
#plt.ylim(0,4e9)
plt.xlabel('Energy (MeV)')
plt.ylabel('d2N/dOmegadE')
plt.title('Electron Energy Spectrum')
plt.grid(which="both", lw=1, color="k")
plt.show()
