...
plt.errorbar(x=delay1[0:len(peaks1)-1],y=peaks1[0:len(peaks1)-1]-410.4, yerr=np.ones(len(peaks1[0:len(peaks1)-1]))*0.074,color='b', capsize=1, linewidth=0.5)
plt.title("Doppler shift")
...
plt.plot(delay1, v1, 'g-', lw=4, label="scan 1")
plt.plot(red_delay1, red_v1, 'ro')
...
plt.plot(delay2, v2, 'y-', lw=4, label="scan 2")
plt.plot(red_delay2, red_v2, 'rd')
plt.plot(blue_delay2, blue_v2, 'bd')
plt.errorbar(x=delay2, y=v2, yerr=[v2_lerr,v2_uerr],  elinewidth=0.5, capsize=2, capthick=0.5, color = 'k')
plt.plot(delay2, v2, 'k-',lw=1)


plt.title("Pu-Pr delay vs dopler velocity at: "+r"$I=1\times 10^{18}\ W/cm^2$"+"\n(red means inside the target velocity)")
plt.grid(lw = 0.5, color = "black")
plt.xlabel("""Probe delay (ps)""", size = 8)
plt.ylabel(r"v (cm/s)")
...
# Set background colors based on y-values
plt.fill_between(x, y2, where=(y2 <= 0), color='b', alpha=0.3)
plt.fill_between(x, y1, where=(y1 > 0), color='r', alpha=0.3)
plt.xlim(-5,15.3)
plt.xticks(size = 8)
plt.yticks(size = 8)
plt.legend()
...
data1 = np.column_stack((delay1[1:-1], v1[1:-1], v1_uerr[1:-1]))
data2 = np.column_stack((delay2[1:-1], v2[1:-1], v2_uerr[1:-1]))

# Open a file to write
with open("D:\\output.txt", "w") as f:
    # Write the first section
    f.write("""Intensity: 1e17 W/cm^2
...
Scan 2
delay | velocity | error_v\n""")
    np.savetxt(f, data2, fmt="%.2f", delimiter=", ")
