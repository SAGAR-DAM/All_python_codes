...
def laser(x,t):
    return 1*np.exp(-(x-t)**2/0.1) * np.cos(100 * x)

# Set up the 3D plot
fig = plt.figure(figsize=(8, 6))
ax = fig.add_subplot(111, projection='3d')
ax.view_init(elev=20, azim=0)  # adjust angles for different view
...
def update(frame):
    ...
    # ax.set_title(r"velocity (in cm/s) map for I = $3\times 10^{1} W/cm^2$"+f"\ndelay = {delay[frame]:.2f} ps")
    ax.set_title(r"Critical surface (in $\mu m$) position for I = $3\times 10^{17} W/cm^2$"+f"\ndelay = {delay[frame]:.2f} ps;  critical surf pos = {amp[frame]:.3f}" + r" $\mu$m; velocity = " + f"{velocity[frame]:.1e} cm/s")
    # Reapply axis settings in update
    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_zticks([])
...
# ani.save('D:\\data Lab\\400 vs 800 doppler experiment\\400 pump 400 probe\\14th Feb 2024\\crit_surf animation I_3_17_with_laser.gif', writer='pillow', fps=10)
# Show the plot
plt.show()
