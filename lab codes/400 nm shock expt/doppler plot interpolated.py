...
def plot_file(file1,labelname = "pr_only"):
    ...
    interpolator = interp1d(wavelength, intensity, kind='linear')  # Use 'linear' or 'cubic' depending on smoothness needed
    intensity_dense = interpolator(wavelength_dense)
    
    fit_I,parameters,string = Gf.Gaussfit(wavelength_dense, intensity_dense)
    
    plt.plot(wavelength_dense, intensity_dense,label=labelname)
    # plt.plot(wavelength, fit_I, label=labelname)
    
    peak = np.mean(wavelength_dense[intensity_dense>0.95])
...
plt.ylabel("Relative intensity (arb unit)")
