...
plt.plot(delay1[0:len(peaks1)-1],peaks1[0:len(peaks1)-1]-410.4, 'ro', label = 'scan 1')
plt.plot(delay1[0:len(peaks1)-1],peaks1[0:len(peaks1)-1]-410.4, 'k-')
plt.errorbar(x=delay1[0:len(peaks1)-1],y=peaks1[0:len(peaks1)-1]-410.4, yerr=np.ones(len(peaks1[0:len(peaks1)-1]))*0.074,color='b', capsize=1, elinewidth=1)
plt.plot((delay1[0:len(peaks1)])[peaks1-410.4<=0],peaks1[peaks1-410.4<=0]-410.4, 'bo')
plt.title("Doppler shift")
plt.xlabel("probe delay (ps)",size = 20)
plt.ylabel("Peak wavelength (nm)", size = 20)
plt.xticks(size = 20)
plt.yticks(size = 20)
# plt.xlim(-5,18)
# plt.ylim(-0.125,0.5)
...
plt.plot(delay2[0:len(peaks2)],peaks2[0:len(peaks2)]-410.3, 'md', label = 'scan 2')
plt.plot(delay2[0:len(peaks2)],peaks2[0:len(peaks2)]-410.3, 'k-')
plt.errorbar(x=delay2[0:len(peaks2)-1],y=peaks2[0:len(peaks2)-1]-410.3, yerr=np.ones(len(peaks2[0:len(peaks2)-1]))*0.074,color='k', capsize=1, elinewidth=1)
plt.plot((delay2[0:len(peaks2)])[peaks2-410.3<=0],peaks2[peaks2-410.3<=0]-410.3, 'cd')
plt.title("Doppler shift")
...
# plt.xlim(-5,15)
# plt.ylim(-0.125,0.5)
plt.title("Doppler Shift vs delay\n"+fr"Intensity: 1.3$\times$"+r"$10^{18}$ W/cm$^2$ (p-pol)")
plt.grid(lw = 1, color = "black")

# Generate sample data
x = np.linspace(-5, 16, 100)
y1 = np.ones(len(x))*0.5
y2 = -y1*0.2
...
# Set background colors based on y-values
plt.fill_between(x, y2, where=(y2 <= 0), color='red', alpha=0.2)
plt.fill_between(x, y1, where=(y1 > 0), color='blue', alpha=0.2)
# plt.xlim(-5,15.3)
plt.xticks(size = 20)
plt.yticks(size = 20)
plt.show()
