...
peaks = []
delay = np.linspace(12,15,len(files_16)//2)-12.65
delay = 2*delay/c
delay = np.around(delay, decimals=3)
...
plt.plot(delay[0:len(peaks)-1],peaks[0:len(peaks)-1]-410.30, 'ro')
plt.plot(delay[0:len(peaks)-1],peaks[0:len(peaks)-1]-410.30, 'k-')
plt.title("Doppler shift")
plt.xlabel("probe delay (ps)")
plt.ylabel("Peak wavelength (nm)")
# plt.xlim(-5,18)
