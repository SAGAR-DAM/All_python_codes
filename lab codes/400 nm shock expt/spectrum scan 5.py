...
files = glob.glob("D:\\data Lab\\400 vs 800 doppler experiment\\400 pump 400 probe\\14 th march 2024\\Scan5\\*.txt")
# files2 = glob.glob("D:\\data Lab\\400 vs 800 doppler experiment\\400 pump 400 probe\\14 th march 2024\\Scan4\\e*.txt")
# files = files1+files2


delay = np.linspace(10,14,len(files)//4)-11.35
delay = 2*delay/c
...
for i in range(2,len(files)-2,4):
    # if(i<=161):
    #     f = open(files[i])
    # else:
    #     f = open(files[i-1])
