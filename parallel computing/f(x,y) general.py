...
@np.vectorize
def g(x,y):
    val =np.abs(np.cos(15*(x+y))+np.cos(15*(x-y)))
    return val

# Define the function to be parallelized
def parallel_func(args):
    x_start, x_end, y_start, y_end = args
    Z = g(X[y_start:y_end, x_start:x_end], Y[y_start:y_end, x_start:x_end])
    return Z
...
if __name__ == '__main__':
    ...
    # Plot the result
    plt.imshow(np.abs(Z), cmap="hot", extent=[*x_range, *y_range])
    plt.colorbar()
    plt.show()
